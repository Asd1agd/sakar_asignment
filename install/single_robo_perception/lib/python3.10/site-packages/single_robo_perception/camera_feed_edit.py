...
class CameraFeed(Node):
    def __init__(self, transformer_node):    # , transformer_node
    ...
    def publish_point(self, x_w, y_w, z_w):
        ...
        """
        msg = ClusterPoints()
        msg.points_arr = [float(x_w), float(y_w), float(z_w)]
    ...
    def publish_COM(self, nearest_cluster_centroid_point, id):
        ...
        self.get_logger().info(f'Published COM: ID={id}, point=({msg.com_arr[0]:.2f}, {msg.com_arr[1]:.2f}, {msg.com_arr[2]:.2f})')  # LOG (changed from debug to info)
...
def main(args=None):
    ...
    # Create both nodes
    transformer_node = CameraToWorldTransformer('camera_link', 'map')
    camera_node = CameraFeed(transformer_node)   # pass transformer if needed

    # Use a single-threaded executor (runs all callbacks in the main thread)
